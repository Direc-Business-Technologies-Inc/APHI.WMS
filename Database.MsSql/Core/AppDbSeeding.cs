...
{
    // Splits a T-SQL script on standalone GO batch separators. GO is a client-tool
    // construct (SSMS/sqlcmd), not T-SQL — Dapper/SqlClient cannot execute it, and
    // CREATE OR ALTER PROCEDURE must be the first statement in its own batch.
