...
public partial class GoodsReturnDraftsGrid
{
    ...
    async Task<DataGridResultVM<GoodsReturnDataGridVM>> LoadDataAsync(DataGridIntent intent)
    {
        var action = await AppActionFactory.RunAsync(async () =>
        {
            ...
            var response = await GoodsReturnHandler.GetGoodsReturnDraftDataGridAsync(intent);
    ...
    async Task OnSearchAsync() => await GoodsReturnDataGrid.DataGrid.Reload();

    void ViewGoodsReturn(GoodsReturnDataGridVM purchaseOrder) => NavManager.NavigateTo($"/transactions/purchasing/goods-return/view?ref={purchaseOrder.DocEntry}&draft=true", true);
