...
public partial class SalesReturnDraftsGrid
{
    ...
    async Task<DataGridResultVM<SalesReturnDataGridVM>> LoadDataAsync(DataGridIntent intent)
    {
        var action = await AppActionFactory.RunAsync(async () =>
        {
            ...
            var response = await SalesReturnHandler.GetSalesReturnDraftDataGridAsync(intent);
            return response;
    ...
    async Task OnSearchAsync() => await SalesReturnDataGrid.DataGrid.Reload();

    void ViewSalesReturn(SalesReturnDataGridVM item) => NavManager.NavigateTo($"/transactions/sales/sales-return/view?ref={item.DocEntry}&draft=true", true);
