...
public partial class ApprovedDataGrid
{
    ...
    async Task<DataGridResultVM<GoodsReceiptDataGridVM>> LoadDataAsync(DataGridIntent intent)
    {
        var action = await AppActionFactory.RunAsync(async () =>
        {
            ...
            var response = await GoodsReceiptHandler.GetGoodsReceiptDataGridAsync(intent);
    ...
    void ViewGoodsReceipt(GoodsReceiptDataGridVM purchaseOrder) => NavManager.NavigateTo($"/transactions/inventory/goods-receipt/view?ref={purchaseOrder.DocEntry}", true);
