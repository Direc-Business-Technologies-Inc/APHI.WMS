...
public partial class ApprovedDataGrid
{
    ...
    async Task<DataGridResultVM<GoodsIssueDataGridVM>> LoadDataAsync(DataGridIntent intent)
    {
        var action = await AppActionFactory.RunAsync(async () =>
        {
            ...
            var response = await GoodsIssueHandler.GetGoodsIssueDataGridAsync(intent);
    ...
    void ViewGoodsIssue(GoodsIssueDataGridVM purchaseOrder) => NavManager.NavigateTo($"/transactions/inventory/goods-issue/view?ref={purchaseOrder.DocEntry}", true);
